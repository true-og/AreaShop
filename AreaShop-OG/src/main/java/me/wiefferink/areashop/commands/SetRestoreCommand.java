...
public class SetRestoreCommand extends AreashopCommandBean {
    ...
    private CompletableFuture<Iterable<Suggestion>> suggestRestoreType(@Nonnull CommandContext<CommandSender> context,
            @Nonnull CommandInput input)
    {

        String text = input.peekString();
        List<Suggestion> suggestions = RESTORE_TYPES.stream().filter(type -> type.startsWith(text))
            ...
            @Nonnull CommandContext<CommandSender> context, @Nonnull CommandInput input)
    {
        ...
        String text = input.peekString();
        List<Suggestion> suggestions = schemProfiles.getKeys(false).stream().filter(profile -> profile.startsWith(text))
                .map(Suggestion::suggestion).toList();
        return CompletableFuture.completedFuture(suggestions);
