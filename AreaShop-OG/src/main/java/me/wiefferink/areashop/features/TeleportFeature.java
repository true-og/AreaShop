...
public class TeleportFeature extends RegionFeature {
     ...
     * @param toSign            true to teleport to the first sign of the region,
     *                          false for teleporting to the region itself
     * @param checkRestrictions Set to true if teleport permissions should be
     *                          checked, false otherwise, also toggles cross-world
     *                          check
    ...
    public boolean teleportPlayer(Player player, boolean toSign, boolean checkRestrictions) {
        ...
        if (checkRestrictions) {
            ...
            // Check correct world
            if (!getRegion().getBooleanSetting("general.teleportCrossWorld")
                    && !player.getWorld().equals(getRegion().getWorld()))
            {
            ...
            // Teleport to sign instead if they dont have permission for teleporting to
            // region
            if ((!toSign && owner && !player.hasPermission("areashop.teleport")
                    && player.hasPermission("areashop.teleportsign")
                    || !toSign && !owner && !friend && !player.hasPermission("areashop.teleportall")
                            && player.hasPermission("areashop.teleportsignall")
                    || !toSign && !owner && friend && !player.hasPermission("areashop.teleportfriend")
                            && player.hasPermission("areashop.teleportfriendsign")
                    || !toSign && !owner && !friend && available && !player.hasPermission("areashop.teleportavailable")
                            && player.hasPermission("areashop.teleportavailablesign")))
            {
            ...
            } else if (!owner && !available && !player.hasPermission("areashop.teleportfriendsign") && toSign
                    && friend)
            {
        ...
        boolean blocksInRegion = worldguardRegion.contains(startLocation.getBlockX(), startLocation.getBlockY(),
        ...
        boolean northDone = false, eastDone = false, southDone = false, westDone = false, topDone = false,
                bottomDone = false;
        ...
        while ((blocksInRegion || !insideRegion) && !done) {
            ...
            for (int x = -radius + 1; x <= radius && !done && !northDone; x++) {
                ...
                for (int y = -radius + 1; y < radius && !done; y++) {
                    ...
                    if (worldguardRegion.contains(safeLocation.getBlockX(), safeLocation.getBlockY(),
                            safeLocation.getBlockZ()) || !insideRegion)
                    {
     ...
     * 
     * @param player Player that should be teleported
     * @param toSign true to teleport to the first sign of the region, false for
     *               teleporting to the region itself
     * @return true if the teleport succeeded, otherwise false
     */
    ...
    private boolean isSafe(Location location) {
        ...
        // Get all blocks around the player (below foot level, foot level, head level
     ...
     * 
     * @param material Material to check (assumed that this is at the feet or head
     *                 level)
     * @return true when it is safe to spawn inside, otherwise false
     */
    private static boolean canSpawnIn(Material material) {

        return material == Material.DRAGON_EGG || Tag.DOORS.isTagged(material) || Tag.SIGNS.isTagged(material)
    ...
    private static boolean cannotSpawnOn(Material material) {
        ...
        return material == Material.CACTUS || material == Material.PISTON || material == Material.PISTON_HEAD
                || material == Material.MOVING_PISTON || material == Material.DRAGON_EGG
                || material == Material.MAGMA_BLOCK || material == Material.REDSTONE_LAMP
                || Tag.SIGNS.isTagged(material) || Tag.DOORS.isTagged(material) || Tag.FENCE_GATES.isTagged(material)
                || Tag.FENCES.isTagged(material) || material.name().contains("GLASS_PANE");
     ...
     * 
     * @param material Material to check (assumed that this is somewhere around the
     *                 player)
     * @return true when it is safe to spawn next to, otherwise false
     */
    private static boolean cannotSpawnBeside(Material material) {

        return material == Material.LAVA || material == Material.FIRE || material == Material.MAGMA_BLOCK;
     ...
     * @param toSign true to try teleporting to the first sign, false for
     *               teleporting to the region
    ...
    private Location getStartLocation(Player player, Value<Boolean> toSign) {
        ...
        if (toSign.get()) {
            ...
            if (signAvailable) {
                ...
                if (distance > 0) {
                    ...
                    Vector facingVector = new Vector(facing.getModX(), facing.getModY(), facing.getModZ()).normalize()
                            .multiply(distance);
        ...
        if (startLocation == null) {
            ...
            startLocation = new Location(getRegion().getWorld(), middle.getX(), middle.getY(), middle.getZ(),
