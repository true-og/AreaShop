...
public class ASLanguageManager extends LanguageManager {
    ...
    public ASLanguageManager(JavaPlugin plugin, String jarLanguagePath, String currentLanguageName,
            String defaultLanguageName, List<String> chatPrefix, String wgPrefix)
    {
    ...
    private void saveDefaults() {
        ...
        try {
            ...
            jarPath = new File(plugin.getClass().getProtectionDomain().getCodeSource().getLocation().toURI());
        ...
        try (ZipFile jar = new ZipFile(jarPath);) {
            ...
            while (entries.hasMoreElements()) {
                ...
                if (!entry.isDirectory() && entry.getName().startsWith(jarLanguagePath + "/")
                        && entry.getName().endsWith(".yml"))
                {

                    // Save the file to disk
                    File targetFile = new File(languageFolder.getAbsolutePath() + File.separator
                            ...
                            OutputStream output = new FileOutputStream(targetFile))
                    {
                        ...
                        Log.warn(
                                "Something went wrong saving a default language file: " + targetFile.getAbsolutePath());
    ...
    private Map<String, List<String>> loadLanguage(String key, boolean convert) {
                ...
                Reader reader = new InputStreamReader(inputStream, StandardCharsets.UTF_8))
        {
            ...
            // Detect empty language files, happens when the YAML parsers prints an
            // exception (it does return an empty YamlConfiguration though)
        ...
        if (convertFromTransifex) {
            ...
            if (!Transifex.convertFrom(file)) {
                ...
                Log.warn("Failed to convert " + file.getName()
                        + " from the Transifex layout to the AreaShop-OG layout, check the errors above");
