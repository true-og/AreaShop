...
/**
 * Main class for the AreaShop plugin. Contains methods to get parts of the
 * plugins functionality and definitions for constants.
 */
public final class AreaShop extends JavaPlugin implements AreaShopApi {
    ...
    public static final String tagPlayerColor = "playercolor";
    ...
    public void onEnable() {
        ...
        if (currentServerVersion.versionData().isOlderThan(VersionUtil.MC_1_19_4)) {

            error("Unsupported minecraft version: " + currentServerVersion + "! Minimum is 1.19.4");
            shutdownOnError();
            return;
        ...
        try {
            ...
            Class<?> adapterImpl = Class
                    .forName("me.wiefferink.areashop.adapters.platform.modern.ModernPlatformAdapter");
        ...
        String weHandlerVersion = "WorldEditHandler";
        ...
        DependencyModule dependencyModule = new DependencyModule(this.worldEdit, this.worldGuard, getEconomy(),
        ...
        try {
            ...
            if (WorldEditInterface.class.isAssignableFrom(clazz)) { // Make sure it actually implements
                ...
                worldEditInterface = (WorldEditInterface) clazz.getConstructor(AreaShopInterface.class)
                        .newInstance(this); // Set our handler

            }

        } catch (Exception e) {

            error("Could not load the handler for WorldEdit (tried to load " + weHandlerVersion
                    + "), report this problem to the author: " + ExceptionUtils.getStackTrace(e));
            shutdownOnError();
            return;
        ...
        try {
            ...
            if (WorldGuardInterface.class.isAssignableFrom(clazz)) { // Make sure it actually implements
                ...
                worldGuardInterface = (WorldGuardInterface) clazz.getConstructor(AreaShopInterface.class)
                        .newInstance(this); // Set our handler
            ...
            error("Could not load the handler for WorldGuard, report this problem to the author:"
        ...
        AreaShopModule asModule = new AreaShopModule(this, this.messageBridge, this.platformAdapter,
                this.worldEditInterface, this.worldGuardInterface, this.signErrorLogger, this.serviceManager,
                platformModule, dependencyModule);
        ...
        try (java.util.zip.ZipInputStream zip = new java.util.zip.ZipInputStream(
                new java.io.FileInputStream(this.getFile())))
        {
        ...
        if (getConfig().getBoolean("checkForUpdates")) {
            ...
            githubUpdateCheck = new GithubUpdateCheck(this, "true-og", "AreaShop-OG")
                    .withVersionComparator((latestVersion, currentVersion) ->
                    {
                        ...
                        AreaShop.info("Update from AreaShop-OG V" + cleanVersion(result.getCurrentVersion())
                                + " to AreaShop-OG V" + cleanVersion(result.getLatestVersion())
                                + " available, get the latest version at https://github.com/true-og/AreaShop-OG/releases");
    ...
    /**
     * Notify a player about an update if he wants notifications about it and an
     * update is available.
     * 
    ...
    public void notifyUpdate(CommandSender sender) {
        ...
        if (githubUpdateCheck != null && githubUpdateCheck.hasUpdate()
                && sender.hasPermission("areashop.notifyupdate"))
        {

            messageBridge.message(sender, "update-playerNotify", cleanVersion(githubUpdateCheck.getCurrentVersion()),
    ...
    /**
     * Set if the plugin is ready to be used or not (not to be used from another
     * plugin!).
     * 
    ...
    private void setupLanguageManager() {
        ...
        languageManager = new ASLanguageManager(this, languageFolder,
                useMiniMessage() ? getConfig().getString("mmLanguage") : getConfig().getString("language"), "EN",
     ...
     * 
     * @param chatprefix The string to use in front of chat messages (supports
     *                   formatting codes)
     */
    ...
    private DiamondBankAPIJava getEconomy() {
        ...
        RegisteredServiceProvider<DiamondBankAPIJava> economy = getServer().getServicesManager()
     ...
     * @return true if the player has the permission, false if the player does not
     *         have permission or, is offline and there is no LuckPerms provider
     *         available
     */
    public boolean hasPermission(OfflinePlayer offlinePlayer, String permission) {
        ...
        // Player offline and no offline permission provider available, safely say that
    ...
    /**
     * Method to get the FileManager (loads/save regions and can be used to get
     * regions).
     * 
    ...
    private void setupTasks() {
        ...
        Bukkit.getScheduler().runTaskTimerAsynchronously(this, () -> signErrorLogger.saveIfDirty(), signLogTicks,
                signLogTicks);
     ...
     * 
     * @param message Object parts of the message that should be logged, toString()
     *                will be used
     */
    @Override
     ...
     * 
     * @param confirmationReceiver The CommandSender which should be notified when
     *                             complete, null for nobody
     */
