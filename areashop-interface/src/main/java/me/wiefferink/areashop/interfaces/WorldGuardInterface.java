...
public abstract class WorldGuardInterface {
    ...
    // DefaultDomain changed from abstract to non-abstract in version 6, so we need
    // to handle that differently
    /**
     * Parse an owner(s) string and set the players as owner of the WorldGuard
     * region (set by UUID or name depending on implementation).
     * 
    ...
    /**
     * Parse a member(s) string and set the players as member of the WorldGuard
     * region (set by UUID or name depending on implementation).
     * 
     ...
     * @return RegionAccessSet with all members (by uuid and name) and groups of the
     *         given region
     */
     ...
     * @return RegionAccessSet with all owners (by uuid and name) and groups of the
     *         given region
     */
    public abstract RegionAccessSet getOwners(ProtectedRegion region);

    // New flag system was introduced in version 6.1.3, requiring different flag
    // parsing
    /**
